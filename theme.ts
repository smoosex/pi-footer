...
/**
 * Get raw ANSI foreground color code without wrapping text or reset.
 * Use when you need just the color escape sequence (e.g., separators
 * between independently-colored segments).
 */
export function getColorCode(
  ...
  const color = resolveColor(semantic, presetColors);
  if (isHexColor(color)) return hexToAnsi(color);

  // ThemeColor: extract the raw ANSI code by wrapping a sentinel
  const sentinel = "\x00";
  const wrapped = applyColor(theme, color, sentinel);
  const idx = wrapped.indexOf(sentinel);
  return idx > 0 ? wrapped.slice(0, idx) : "";
}
