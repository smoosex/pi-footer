...
let isNotGitRepo = false;
let lastGitCheckTime = 0;
const RETRY_NON_GIT_MS = 30000;

let onCacheUpdate: (() => void) | null = null;

export function setGitCacheUpdateCallback(cb: (() => void) | null): void {
  onCacheUpdate = cb;
}
...
export function getGitStatus(providerBranch: string | null): GitStatus {
  ...
  if (isNotGitRepo && now - lastGitCheckTime < RETRY_NON_GIT_MS) {
  ...
  if (!pendingFetch) {
    ...
    pendingFetch = fetchGitStatus().then((result) => {
      if (fetchId === invalidationCounter) {
        if (result === null) {
          isNotGitRepo = true;
          cachedStatus = { staged: 0, unstaged: 0, untracked: 0, timestamp: Date.now() };
        } else {
          isNotGitRepo = false;
          cachedStatus = {
